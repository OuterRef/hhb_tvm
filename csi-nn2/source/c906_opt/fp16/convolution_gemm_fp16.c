                                    ...
                                    struct csinn_conv2d_params *params)
{
    ...
    int32_t pad_if_zero = pad_left + pad_top;
    ...
    if (pad_if_zero) {
        for (int i = 0; i < batch; i++) {
            for (int g = 0; g < group; g++) {
                ...
                shl_c906_reorder_input_fp16_1(im2col_data, pb, k, n, n);
                // GEMM
                shl_c906_sgemm_kernel_fp16_fuse_relu(pc, pa, pb, m, k, n, n, bias_data + g * m);
                input_data += in_ch / group * in_height * in_width;
                output_data += m * n;
            }
        }
        ...
        for (int i = 0; i < batch; i++) {
            for (int g = 0; g < group; g++) {
                ...
                // shl_c906_reorder_input_fp16_1(im2col_data, pb, k, n, n);
                // GEMM
                shl_rvv_gemm_8x16_fp16_fuse_relu(pc, pa, pb, bias_data + g * m, m, k, n, n);
                // shl_c906_sgemm_kernel_fp16(pc, pa, pb, m, k, n, n, bias_data + g * m);
                input_data += in_ch / group * in_height * in_width;
                output_data += m * n;
            }
